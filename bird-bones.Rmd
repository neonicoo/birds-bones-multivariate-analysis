...
Les valeurs propres nous renseignent sur la fraction de l’inertie totale prise en compte par chaque axe. Elles sont rangées par convention dans l’ordre décroissant. Le premier axe représente 87 % de l'inertie totale et le deuxième axe représente 6.5% de l'inertie. Si l'on ne conserve que ces deux valeurs propres là, leur somme (ou cumul) étant égale à 93% est suffisante pour représenter l'inertie totale, il est possible de représenter la projection de nos 10 variables dans un repère définit par nos 2 axes principaux grâce à la commande *s.corcircle*. 
...
wdis <- lda.birds$fa / sqrt( sum(lda.birds$fa**2))    # normalised weights
...
A présent, et pour la suite de ce devoir nous désirons afficher les résultats des différentes analyses sous forme de tableau de la forme [classes * variables]. En d'autres termes cela signifie avoir pour chaque classe d'individus, la moyenne de tous les individus pour chaque variable. Cela nous servira par exemple à calculer des matrices de distance entre classes d'individus.
...
Pour la BCA, le tableau et directement fournie par la fonction. On remarque qu'il s'agit du même tableau que celui que nous avons construit à partir des résultats de l'ACP, ce qui est cohérent puisque la BCA est un cas particulier de l'ACP. 
...
```{r plots pco, fig.height=2, fig.width=2.5, echo=FALSE}
...
À présent nous allons essayer de caractériser les 6 types d'oiseaux présents dans le jeu de donnée selon les différentes variables de tailles en utilisant tous les résultats d'analyses obtenus jusqu'ici. En cas d'ambiguïté nous privilégierons les résultats de la BCA plutôt que ceux l'ADL.
...
- Les classes d'oiseaux raptors (R) et d'oiseaux nageurs (SW) sont elles aussi assez proches sur nos graphiques. Elles notamment caractérisées par la longueur du tibiotarse et du tarso-métatarse, soit aux parties basses des membres inférieurs. 

- La classe des oiseaux terrestres (T) est caractérisé par la longueur et le diamètre du fémur, mais également un peu par le diamètre du tarso-métatarse.

- Enfin la dernière classe d'oiseaux échassiers (W) est caractérisée par la longueur et le diamètre des tibiotarse mais aussi par le diamètre du fémur.
