...
```{r pco, warning=FALSE}
pco.birds_bet <- dudi.pco(dist(bcatab)**2, scannf = FALSE, nf = 2)
pco.birds_discrim <- dudi.pco(dist(ldatab)**2, scannf = FALSE, nf = 2)
pco.variables <- dudi.pco(dist(t(birds2[,-11]))**2, scannf = FALSE, nf = 2)
```
...
```{r clustering }
clusty_bet <- hclust(dist(bcatab)**2, "complete")
clusty_discrim <- hclust(dist(ldatab)**2, "complete")
clusty_var <- hclust(dist(t(birds2[,-11]))**2, "complete")
```
