...
```{r centrage réduction, fig.width=10, echo=FALSE}
...
## 2. Between-Class Analysis (BCA) :

Après avoir effectuer notre Analyse en Composante Principale nous pouvons approfondir notre étude avec un cas particulier de cette ACP, il s'agit de faire une Analyse Interclasse (en anglais Between-Class Analysis BCA). Une ACP permet dans un premier temps de rechercher une combinaison linéaire de variables maximisant la variance totale des données, une BCA permet  également de rechercher la meilleure combinaison linéaire mais qui cette fois maximise la variance entre classes.

Ainsi cette analyse va nous donner la possibilité de superposer la variable "groups" contenant les groupes écologiques de chaque individu, sur notre ACP. Avec ça nous verrons quelles classes d'individus sont proches ou éloignées les unes des autres, mais également  quels sont les attributs (longueur ou largeur d'os) qui peuvent caractériser une classe plûtot qu'un autre.
...
ldatab <- aggregate( t( (as.matrix(lda.birds$fa) %*% t(as.matrix(lda.birds$li)) ) ), 
...
Voici un tableau (dataframe) qui contient les moyennes de chaque variables , pour chaque classe. On rappel que nos données ont subit un centrage-réduction, c'est pour cela qu'il y a des valeurs négatives.

Par ailleurs il est possible d'obtenir un tel tableau à partir de nos données brutes, sans effectuer d'ACP. En utilisant la fonction *aggregate()* il est possible de regrouper tous les individus par classe respective, et d'obtenir les moyennes correspondantes  à chaque variable :

On s'aperçois que certaines valeurs de moyennes sont proches et d'autre moins entre *meantab* et *bcatab*. Pour la suite de notre devoir il sera plus intéressant pour nous d'utiliser *bcatab*, pour calculer des distances par exemple, car celui est issus d'une ACP et maximise donc au mieux les variances entre chaque classe (tout en maximisant l'inertie totale). 

Remarque : Il serait également possible de faire une table basée sur les médianes plutôt que les moyennes (pas d'intérêt ici). 

## 5. Distances et clustering :

```{r clustering }
dbirds_bet <- round(dist(bcatab), 3)
dbirds_discrim <- round(dist(ldatab), 3)

pco.birds_bet <- dudi.pco(dbirds_bet, scannf = FALSE)
pco.birds_discrim <- dudi.pco(dbirds_discrim, scannf = FALSE)

clusty_bet <- hclust(dbirds_bet, "complete")
clusty_discrim <- hclust(dbirds_discrim, "complete")
```

## 6. Affichages :

```{r s.arrow(), fig.height=2.25, fig.width=2.25, echo=FALSE}
par(mfrow=c(1,3))
s.arrow(pca.birds$c1)
s.arrow(bca.birds$c1)
s.arrow(lda.birds$fa)
```

```{r s.class, fig.height=2.25, fig.width=2.25, echo=FALSE}
par(mfrow=c(1,3))
s.class(pca.birds$li, as.factor(birds2$groups))
s.class(bca.birds$ls,as.factor(birds2$groups))
s.class(lda.birds$li,as.factor(birds2$groups))
```


```{r dendrograms, echo=FALSE}
par(mfrow = c(1, 2))
...
```{r scatter, fig.height=2.5, fig.width=2.5, echo=FALSE}
par(mfrow=c(1,2))
