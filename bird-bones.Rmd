...
A présent nous désirons afficher les résultats des différentes analyses sous forme de tableau de la forme [classes * variables]. En d'autres termes cela signifie avoir pour chaque classe d'individus, la moyenne de tous les individus pour chaque variable. 
...
Pour la BCA, le tableau et directement fournie par la fonction. On remarque q'uil s'agit du même tableau que celui que nous avons construit à partir des résultats de l'ACP, ce qui est cohérent puisque la BCA est un cas particulier de l'ACP. 
...
dbirds_var <- round((dist(t(birds2[1:10]))), 2)
```




## 5. Analyse Principale en Coordonnées (PCO) :

```{r pco}
pco.birds_bet <- dudi.pco(dbirds_bet, scannf = FALSE, nf = 2)
pco.birds_discrim <- dudi.pco(dbirds_discrim, scannf = FALSE, nf = 2)
pco.variables <- dudi.pco(dbirds_var, scannf = FALSE, nf = 2)
```
